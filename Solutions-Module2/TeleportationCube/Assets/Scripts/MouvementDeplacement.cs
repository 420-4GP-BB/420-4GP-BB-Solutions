...
public class MouvementDeplacement : MonoBehaviour
{
    ...
    private Coroutine deplacement; // On conserve une référence de la coroutine pour pouvoir l'arêter.

    void Start()
    {
        _rbody = GetComponent<Rigidbody>();
        deplacement = StartCoroutine(DeplacerCube(transform.position));
    }
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (positionClic != null)
            {
                ...
                deplacement = StartCoroutine(DeplacerCube(positionFinale));
            }
        }
    }
    ...
    private IEnumerator DeplacerCube(Vector3 positionFinale)
    {
        ...
        while (!termine)
        {
            ...
            if (distance >= 0.1f)
            {
                ...
                Vector3 nouvellePosition = transform.position + (direction * vitesse);
